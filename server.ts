...
async function startServer() {
  ...
  app.post("/api/detect-components", upload.single("image"), async (req, res) => {
    ...
    try {
      ...
      Identify: switches, sockets, fan_regulators, ac_switches, smart_buttons, and dimmers.
      Return the counts in a structured JSON format.`;
            ...
            properties: {
              components: {
                ...
                items: {
                  type: Type.OBJECT,
                  properties: {
                    type: { type: Type.STRING, description: "The type of component (switch, socket, fan_regulator, ac_switch, smart_button, dimmer)" },
